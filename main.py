...
def box_seg(parent, options, current, on_select, size=11, btn_w=0):
    ...
    def _sel(val):
        ...
        for v, b in btns.items():
            active = v == val
            b.configure(border_color=BORDER if active else PANEL)
    for label, val in options:
        active = val == current
        b = ctk.CTkButton(
            row, text=label, width=btn_w, height=28, corner_radius=4,
            fg_color=PANEL, hover_color=PANEL,
            border_color=BORDER if active else PANEL, border_width=1,
            text_color=MUTED,
...
class App(ctk.CTk):
    ...
    def _build_settings_view(self):
        ...
        def _build_timer(parent):
            ...
            _section(f, "Max Pomo Duration")
            preset_vals = [25, 45, 60, 90, 120]
            preset_btns = {}

            def _select_preset(v, reset_custom=True):
                self._pomo_max_mins = float(v)
                save_settings("pomo_max_mins", float(v))
                for pv, pb in preset_btns.items():
                    active = pv == v
                    pb.configure(
            ...
            def _apply_custom(*_):
                ...
                try:
                    v = float(val)
                    if v > 0:
                        for pb in preset_btns.values():
                            pb.configure(text_color=MUTED,
                                         font=ctk.CTkFont(size=12, weight="normal"))
                        custom_entry.configure(border_color=DARK)
                        _select_preset(v, reset_custom=False)
                    else:
                        raise ValueError
                except ValueError:
                    custom_entry.configure(border_color=DANGER)
            ...
            _section(f, "Timer Direction")
            _seg(f, [("Fill", True), ("Empty", False)], self._timer_fills,
                 lambda v: (setattr(self, "_timer_fills", v),
                            save_settings("timer_fills", v)))

            _section(f, "Day ends at")
            _seg(f,
                 [(str(h), h) for h in [22, 23, 0, 1, 2, 3, 4, 5]],
                 load_settings().get("day_end_hour", 3),
                 lambda v: save_settings("day_end_hour", v))
        ...
        def _build_data(parent):
            f = ctk.CTkFrame(parent, fg_color="transparent")
            _section(f, "Database")
            _cur_db = "newui" if config.DB_FILE == DB_NEWUI else "main"
            db_btns = _seg(f, [("DB New UI","newui"),("DB Main","main")], _cur_db,
                           lambda k: self._switch_db(k))

            _section(f, "Log")
            log_row = ctk.CTkFrame(f, fg_color="transparent")
            log_row.pack(anchor="w")
            mk_btn(log_row, "Open Log", self._open_log, width=90, height=26).pack(side="left")
            info_lbl = ctk.CTkLabel(log_row, text="ⓘ", width=18, height=18,
                                    fg_color="transparent", text_color=DIM,
                                    font=ctk.CTkFont(size=11))
            info_lbl.pack(side="left", padx=(6, 0))
            tip_lbl = mk_label(f, "", size=10, color=DIM)
            tip_lbl.pack(anchor="w")
            info_lbl.bind("<Enter>", lambda _: tip_lbl.configure(text="needs a SQLite reader"))
            info_lbl.bind("<Leave>", lambda _: tip_lbl.configure(text=""))
            return f

        def _build_app(parent):
            f = ctk.CTkFrame(parent, fg_color="transparent")
        ...
        if self._skill_edit_mode:
            ...
            for name, emoji in skills:
                ...
                ctk.CTkButton(
                    ...
                    fg_color=PANEL, hover_color=BORDER, text_color=MUTED,
    ...
    def _draw_bars(self, canvas, skill: str, period: str, canvas_w: int, tip_lbl):
        ...
        # Hover — tip drawn inside canvas at bottom margin, centered on bar X
        btw = bar_total_w
    ...
    def _build_ambience_view(self) -> ctk.CTkFrame:
        ...
        for i in range(9):
            ...
            # Hover-icon row is built once, eagerly, right here — never
            # destroyed/recreated later. A CTkFrame/CTkButton created and
            # shown on demand during live interaction (e.g. inside a hover
            # handler) can paint one incorrect (white) frame before
            # customtkinter resolves its real color; building it now, during
            # the tab's initial synchronous construction, means it's already
            # fully drawn long before the user can ever hover it.
            overlay = ctk.CTkFrame(t, fg_color=CARD, corner_radius=0, border_width=0)
            btn_kw = dict(
                width=22, height=20, corner_radius=0,
                fg_color=CARD, hover_color=BG,
                border_width=0, text_color=MUTED, font=ctk.CTkFont(size=10),
            )
            o_btn = ctk.CTkButton(overlay, text="o", command=lambda idx=i: _rename_slot(idx), **btn_kw)
            m_btn = ctk.CTkButton(overlay, text="-", command=lambda idx=i: _flip_face(idx), **btn_kw)
            x_btn = ctk.CTkButton(overlay, text="x", command=lambda idx=i: _delete_slot(idx), **btn_kw)
            o_btn.pack(side="left", padx=5)
            m_btn.pack(side="left", padx=5)
            x_btn.pack(side="left", padx=5)
            overlays.append({"frame": overlay, "o": o_btn, "m": m_btn, "x": x_btn})
        ...
        def _overlay_resting(idx):
            """Baseline (non-hover) visibility: hidden entirely for an empty
            tile or the front face; on the stats face, only '-' shows."""
            ov = overlays[idx]
            slot = slots[idx]
            if slot is None:
                ov["frame"].place_forget()
                return
            if face_state[idx] == "stats":
                ov["o"].pack_forget()
                ov["x"].pack_forget()
                ov["frame"].place(relx=0.5, rely=1.0, anchor="s", y=-8)
            else:
                ov["frame"].place_forget()

        def _overlay_hover_show(idx):
            if slots[idx] is None:
                return
            ov = overlays[idx]
            for b in (ov["o"], ov["m"], ov["x"]):
                b.pack_forget()
            ov["o"].pack(side="left", padx=5)
            ov["m"].pack(side="left", padx=5)
            ov["x"].pack(side="left", padx=5)
            ov["frame"].place(relx=0.5, rely=1.0, anchor="s", y=-8)

        def _bind_tile_hover(tile, idx):
            def _show(_e=None):
                _overlay_hover_show(idx)

            def _maybe_hide():
                if not _pointer_inside(tile):
                    _overlay_resting(idx)
        ...
        for i in range(9):
            _bind_tile_hover(tiles[i], i)

        def _render_tile(idx):
            tile = tiles[idx]
            overlay_frame = overlays[idx]["frame"]
    ...
    def _refresh_today(self):
        ...
        if d["best_day_min"] > 0:
            BAR_H   = 4
            DOT_R   = 5
            BAR_YC  = DOT_R + 2               # = 7, bar vertical center in 14px canvas
